...
describe("getPackageName", () => {
  ...
  it("returns package name without version for scoped packages", () => {
    expect(getPackageName("@scope/example-package@1.2.3")).toBe(
      "@scope/example-package",
    );
    expect(getPackageName("@scope/another-package@4.5.6")).toBe(
      "@scope/another-package",
    );
  });

  it("returns full name for scoped packages without version", () => {
    expect(getPackageName("@scope/example-package")).toBe(
      "@scope/example-package",
    );
    expect(getPackageName("@scope/another-package")).toBe(
      "@scope/another-package",
    );
  });
